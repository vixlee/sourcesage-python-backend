...
from user_api.common import AESCipher, create_db_file
...
def add_user():
    ...
    new_user = User(email, encrypted_password, name, age, gender)
...
def login():
    ...
    user = User.query.filter_by(email=email).first()
    if not user:
        return jsonify({"message": "User User not found"}), 401
